...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    equivalence_selected = frozenset(
        item.nodeid
        for item in items
        if item.get_closest_marker(_EQUIVALENCE_MARKER) is not None
    )
    ...
    if config.getoption("--require-semantic-equivalence"):
        if not equivalence_selected:
            raise pytest.UsageError(
                "--require-semantic-equivalence selected 0 equivalence tests"
            )
    ...
    if selected:
        ...
        if not status.available:
            raise pytest.UsageError(
                "GHDL integration selected but pyGHDL runtime is unavailable: "
                f"{status.detail}"
            )
